---
name: supervisor
description: Autonomous multi-agent quality supervisor and release gate. Use when a user wants the best possible result, asks for rigorous review, or when a substantive answer/deliverable should be challenged before release. Decomposes the task, fans out independent subagents, uses specialist and adversarial judges, verifies current facts on the web when needed, runs tests/visual inspection when possible, loops on defects, and releases only after a final quality gate.
argument-hint: "[task or quality target]"
allowed-tools:
  - WebSearch
  - WebFetch
  - Agent
  ...
  Stop:
    - hooks:
        - type: agent
          ...
          timeout: 120
          prompt: |
            You are SUPERVISOR_STANDALONE_FINAL_JUDGE. Before this skill is allowed to finish, inspect the latest answer and the available evidence from the current turn. $ARGUMENTS

            For substantive work, require: (1) the exact user request is satisfied; (2) material factual/current/technical claims are grounded in user files, runtime evidence, or fresh WebSearch/WebFetch evidence as applicable; (3) no known P0/P1 defect remains; (4) code success claims have build/test/typecheck/runtime evidence when reasonably possible; (5) frontend/visual success claims have rendered/browser/screenshot evidence when reasonably possible; (6) the answer does not pretend subagents, searches, tests, screenshots, or verification ran when they did not; (7) the final response contains a concise Verification Receipt describing web research, local checks, and limitations.

            If a concrete material blocker exists, return {"ok": false, "reason": "specific missing verification or correction"}. Otherwise return {"ok": true}. If stop_hook_active is already true, block again only for a clear unresolved P0/P1, a critical missing requirement, or a materially unsupported claim.
metadata:
  version: "1.0.0"
  tags: [supervisor, multi-agent, quality-gate, review, verification, fan-out]
---
...
Use **real `Agent` tool calls** for independent subagents when the host exposes them. Never say that multiple agents or judges ran if they did not actually run.

If the host does not expose independent subagents, enter `DEGRADED_MODE`: perform distinct inline review passes, label them internally as non-independent, and never misrepresent them as separate agents.

Claude Code subagents cannot recursively spawn more subagents. Therefore all fan-out, cross-review, disagreement resolution, and hierarchical coordination must be orchestrated by the parent conversation. A “hierarchy” is logical delegation by the parent, not recursive spawning.
...
For every substantive answer that contains externally verifiable claims, run at least one real `WebSearch` before release unless the user explicitly forbids browsing or the answer is exclusively a transformation of user-provided material. For current/volatile/technical claims, follow search results with `WebFetch` of the strongest primary/official sources when possible.
